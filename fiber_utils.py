...
def base_reduction(filename, tarfolder=None, get_header=False, from_raw=False):
    '''
    ...
    if tarfolder is not None:
        ...
        if from_raw:
            a = fits.open(T.extractfile('/'.join(filename.split('/')[-6:])))
        else:
...
def get_powerlaw(image, trace, order=3):
    ...
    '''
    def gauss_eval(x1, y1, z1, x2, y2, sigma=50.):
    ...
    for xchunk, tchunk, cnt in zip(XV, T, cnts):
        ...
        for fib in fibgap:
            x, y = ([], [])
            dy = np.array(np.ceil(trace[fib, xchunk])+5, dtype=int)
            dy2 = np.array(np.ceil(trace[fib+1, xchunk])-5, dtype=int)
            for j, xc in enumerate(xchunk):
            ...
            if len(y):
                ...
                avgz.append(np.nanmedian(image[y, x]))
        x, y = ([], [])
        dy = np.array(np.ceil(trace[-1, xchunk])+5, dtype=int)
        for j, xc in enumerate(xchunk):
            d = np.arange(dy[j], image.shape[1]-4)
            if len(d):
                y.append(d)
                x.append([xc] * len(d))
        if len(y):
...
def get_fiber_to_fiber(data, error, wave, sigma=5., width=2):
    ...
    for i in np.arange(data.shape[0]):
        ...
        for j in np.arange(1, int(width+1)):
            mask[i, j:] += mask[i, :-j]
            mask[i, :-j] += mask[i, j:]
    return ftf, mask
