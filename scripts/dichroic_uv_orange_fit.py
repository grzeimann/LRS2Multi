...
def _load_lrs2_spectrum(obj):
    ...
    ----------
    obj : str or object
        A spectrum filename, an object with a ``filename`` attribute, or an
        extracted LRS2Multi-style object with ``wave``, ``spectrum``,
        ``skyspectrum``, and ``normcurve`` attributes.
    ...
    ``data[-1]``
        Response function.
    """
    # LRS2Multi objects contain the extracted native-grid arrays in memory.
    # Prefer them when available so the fit can be run immediately after
    # LRS2Object.extract_spectrum(), before a spectrum FITS product exists.
    if hasattr(obj, "spectrum") and hasattr(obj, "skyspectrum"):
        filename = getattr(obj, "filename", "<in-memory spectrum>")
        wave = np.asarray(obj.wave, dtype=float)
        science = np.asarray(obj.spectrum, dtype=float)
        sky = np.asarray(obj.skyspectrum, dtype=float)
        response = getattr(obj, "normcurve", None)
        if response is None and hasattr(obj, "spec_ext"):
            response = obj.spec_ext[-1]
        if response is None:
            raise ValueError(
                "An in-memory LRS2 spectrum object must provide its response "
                "curve as normcurve or spec_ext[-1]."
            )
        response = np.asarray(response, dtype=float)

        arrays = (wave, science, sky, response)
        if len({array.size for array in arrays}) != 1:
            raise ValueError(
                "The in-memory LRS2 wavelength, science, sky, and response "
